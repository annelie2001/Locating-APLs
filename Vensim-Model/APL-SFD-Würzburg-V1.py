"""
Python model 'APL-SFD-Würzburg-V1.py'
...
@component.add(
    ...
    comp_type="Stateful",
    comp_subtype="Integ",
    depends_on={"_integ_purchases_per_month": 1},
    other_deps={
        "_integ_purchases_per_month": {
            "initial": {"avg_purchase_per_month": 1},
            "step": {"avg_purchase_per_month": 1, "online_purchase_growth_rate": 1},
        }
    },
)
def purchases_per_month():
    return _integ_purchases_per_month()


_integ_purchases_per_month = Integ(
    lambda: avg_purchase_per_month() * online_purchase_growth_rate(),
    lambda: avg_purchase_per_month(),
    "_integ_purchases_per_month",
)


@component.add(
    name='"Avg. purchase per month"',
    units="Units",
    comp_type="Constant",
    comp_subtype="Normal",
)
def avg_purchase_per_month():
    return 5
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"number_of_deliveries": 1, "avg_parcels_per_apl_per_month": 1},
)
def number_of_apls():
    return integer(number_of_deliveries() / avg_parcels_per_apl_per_month())
...
@component.add(
    name='"Avg. parcels per APL per month"', comp_type="Constant", comp_subtype="Normal"
)
def avg_parcels_per_apl_per_month():
    return 6000
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"apl_users": 1, "purchases_per_month": 1},
)
def number_of_deliveries():
    return integer(apl_users() * purchases_per_month())
