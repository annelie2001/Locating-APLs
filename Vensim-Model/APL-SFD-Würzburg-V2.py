"""
Python model 'APL-SFD-Würzburg-V2.py'
...
@component.add(
    name="Online purchases per month",
    units="Units",
    comp_type="Stateful",
    comp_subtype="Integ",
    depends_on={"_integ_online_purchases_per_month": 1},
    other_deps={
        "_integ_online_purchases_per_month": {
            "initial": {"initial_online_purchases_per_month": 1},
            "step": {
                "initial_online_purchases_per_month": 1,
                "online_purchase_growth_rate": 1,
            },
        }
    },
)
def online_purchases_per_month():
    return _integ_online_purchases_per_month()


_integ_online_purchases_per_month = Integ(
    lambda: initial_online_purchases_per_month() * online_purchase_growth_rate(),
    lambda: initial_online_purchases_per_month(),
    "_integ_online_purchases_per_month",
)


@component.add(
    name="Initial online purchases per month",
    units="Units",
    comp_type="Constant",
    comp_subtype="Normal",
)
def initial_online_purchases_per_month():
    return 5
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"number_of_deliveries": 1, "avg_apl_capacity_per_month": 1},
)
def number_of_apls():
    return integer(number_of_deliveries() / avg_apl_capacity_per_month())
...
@component.add(
    name='"Avg. APL capacity per month"', comp_type="Constant", comp_subtype="Normal"
)
def avg_apl_capacity_per_month():
    return 4000
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"apl_users": 1, "online_purchases_per_month": 1},
)
def number_of_deliveries():
    return integer(apl_users() * online_purchases_per_month())
